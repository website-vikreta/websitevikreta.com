...
### [Component] — Breadcrumb `hoverColor` variant + /blog index compact header padding
- Rule: `components/ui/Breadcrumb.tsx` gained an optional `hoverColor?: 'muted' | 'accent'` prop (default `'muted'`, the pre-existing `hover:text-(--color-text)` behavior every other page keeps) rather than hardcoding accent sitewide — a component-param opt-in, not a fork, per the existing [Reuse] convention. Only `/blog` (`app/blog/page.tsx`) passes `hoverColor="accent"` so far.
- `/blog`'s header container padding was cut from the sitewide blog-section value `pt-32 pb-20 md:pt-40 md:pb-28` (still used by `/blog/categories`, `/blog/labels`, `/blog/tags`, `/blog/authors`, `/careers`) down to `pt-20 pb-16 md:pt-24 md:pb-20` — user flagged the top whitespace above the hero slider as excessive on this page specifically. **This is a deliberate, page-scoped divergence, not a new sitewide value** — the other blog landing pages were left untouched. If a future pass wants the compact header everywhere, change all of them together and update this entry rather than letting them drift apart silently.
- Where: `components/ui/Breadcrumb.tsx`, `app/blog/page.tsx`.
- Date: 2026-08-02
