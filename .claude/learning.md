...
- UI: `BlogSearchFilters.tsx` is the only client component — it holds no filter state, just the in-flight text input (debounced 350ms, `router.replace` so keystrokes don't stack history; facet changes use `router.push`). `useTransition` dims the still-mounted results while the next page streams; `useOptimistic` keeps a select/checkbox on its new value instead of snapping back. The input is re-synced from the URL **during render** (`if (syncedQuery !== params.query)`), not in an effect — the effect version both trips `react-hooks/set-state-in-effect` and lets a stale debounce undo an incoming URL. **Any filter change resets `page` to 1.**
