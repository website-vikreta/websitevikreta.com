...
Separately queued, explicitly on hold: GA key-event for the Calendly external-booking CTA in `CalendlyCTABanner.tsx` — user said wait for them to say "take latest" from the release branch (a PR was just merged there with prerequisite changes) before starting.
