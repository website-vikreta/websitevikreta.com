...
Two branches merged together (2026-08-22), both threads DONE and now committed:

**Apps & CRM page (`/services/web-and-mobile-apps`)** — built from a coming-soon placeholder to a full page (commit `da2a551` on `feat/Apps`). `SystemDiagram.tsx`'s scattered→connected motif drives Pain/Solution, a sticky scroll-stack presents the three systems, and `components/ui/how-it-works.tsx` (a reskinned community snippet) renders HowWeWork as a horizontal row with a looped connector. Several proposed directions were tried and explicitly rejected along the way (reusing `AutomationWorkflowCanvas`'s drag UI, a bordered intro panel + heading rewrite, a zigzag connector) — see learning.md's dated 2026-08-22 `[Rejected]`/`[Reverted]` entries before re-proposing any of those.
