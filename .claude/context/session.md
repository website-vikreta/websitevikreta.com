...
**2026-08-22 — Sitewide "book a free audit" popup — DONE, needs verification pass before commit.** Replaced the ai-automations "#book-audit" CTAs' scroll-to-form behavior with a Radix Dialog popup (`components/ui/AuditModal.tsx` + `AuditModalProvider.tsx`, mounted in `app/layout.tsx`). Extracted the existing contact form into a reusable `components/ui/AuditForm.tsx` (used both inline in `ContactSection.tsx` and inside the modal) — added `@radix-ui/react-dialog` dependency. Any `Button`/`TextLink` with `href="#book-audit"` now opens the popup sitewide, not just on this page. Verified via Playwright: validation, EmailJS success/error (mocked), ESC/overlay close, focus trap + refocus-first-field on reopen, body scroll lock, mobile viewport fit, `tsc` clean, `next build` clean. See learning.md [Component] entry. **Not yet committed** — waiting on explicit go per this session's "commit all" pattern.
**2026-08-22 — GA key event for Calendly CTA — DONE.** Merged `releases/2026-08` into this branch first (brought in the `lib/analytics.ts` fix: events now push directly to `window.dataLayer` via a hand-rolled `gtagEvent`, since `sendGAEvent` from `@next/third-parties` silently no-oped — the site hand-rolls the gtag snippet in `app/layout.tsx` on `strategy="lazyOnload"` rather than rendering `<GoogleAnalytics>`, so `sendGAEvent`'s init check always failed). Added a `calendly_click` branch to `getClickEventName()` in `lib/analytics.ts` (matches `href.includes('calendly.com/')`, same pattern as the existing linkedin/instagram checks) — both Calendly CTAs on the ai-automations page (`Hero.tsx`, `CalendlyCTABanner.tsx`) already route external `Button` clicks through `trackLinkClick`, so this one classifier change covers both with no component edits. Previously they fell into the generic `external_link_click` bucket. Verified in a real browser with a simulated `dataLayer`: clicking either CTA pushes `['event', 'calendly_click', {button_location, page_location}]`. `tsc` clean.

**2026-08-22 — WhySection visual rework — DONE (not committed).** User flagged the section's stock watercolor illustration as looking dated/off-brand. Replaced with a coded monochrome checklist card (no new image asset), content unchanged. Old asset deleted. See learning.md [Anti-pattern] "no cartoon/watercolor stock illustrations" — same problem still lives in `PainSection.tsx`'s hero image, not fixed yet (out of scope this pass).
