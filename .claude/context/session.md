...
Apps & CRM service page (`/services/web-and-mobile-apps`) — DONE. Assigned to Vishal in the 2026-08-20 "Services Pages Sync" team meeting (all 4 remaining service pages get independent landing pages sharing one Problem → Solution → Process → Past work → FAQ → Contact structure, split Sanket=Web Dev / Vishal=Apps & CRM / Shubham=AI Automations+UIX+Digital Marketing). Replaced the `ComingSoonPage` stub with: `Hero`, `PainSection`, `FixesSection` (3-card, icon-based — no diagram imagery exists for this service), `ProcessSection` (numbered 01–05 steps) — all new, built on `components/ui/Reveal.tsx` motion primitives (not the `ai-automations` page's one-off GSAP system) — plus `FeaturedWorkSection`/`FaqSection`/`ContactCTASection` reused verbatim (zero new code) for Past work/FAQ/Contact. Full metadata + Service+BreadcrumbList JSON-LD added to `page.tsx`. Verified: `tsc --noEmit` clean, `eslint` clean, `next build` succeeds with the route statically prerendered, and the real HTML response contains the new hero copy/FAQ/schema. **Not verified: real browser/visual check** — no browser automation tool was available in this session, so critic.md's SOTD visual pass and mobile-layout check are still outstanding; do that before calling this launch-ready. See learning.md [Page] and [Content] entries dated 2026-08-21 for the exact structure/reuse decisions — the other 3 pages (Sanket/Shubham) should follow the same pattern, not re-invent it.
