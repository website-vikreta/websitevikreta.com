...
function useFlipCardContext() {
  ...
  if (!context) throw new Error("useFlipCardContext must be used within a FlipCard")
  return context
}
  ...
  React.forwardRef<HTMLDivElement, FlipCardProps>(
    ({ className, flipDirection = "horizontal", initialFlipped = false, onFlip, disabled, ...props }, ref) => {
      ...
      const handleMouseEnter = React.useCallback(() => {
        if (!disabled) { setIsFlipped(true); onFlip?.(true) }
      }, [disabled, onFlip])

      const handleMouseLeave = React.useCallback(() => {
        if (!disabled) { setIsFlipped(false); onFlip?.(false) }
      }, [disabled, onFlip])
      ...
      return (
        <FlipCardContext.Provider value={contextValue}>
          <div
            ref={ref}
            className={cn("relative border-none bg-none shadow-none", disabled && "pointer-events-none", className)}
...
const FlipCardFront = React.memo(
  React.forwardRef<HTMLDivElement, HTMLMotionProps<"div">>(({ className, ...props }, ref) => {
